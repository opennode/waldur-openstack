...
class InstanceCreateTest(test.APITransactionTestCase):
    ...
    def test_quotas_update(self):
        ...
        openstack_spl = self.openstack_settings.scope.service_project_link
        self.assertEqual(openstack_spl.quotas.get(name=openstack_spl.Quotas.ram).usage, instance.ram)
        self.assertEqual(openstack_spl.quotas.get(name=openstack_spl.Quotas.storage).usage, instance.disk)
        self.assertEqual(openstack_spl.quotas.get(name=openstack_spl.Quotas.vcpu).usage, instance.cores)
    ...
    @data('storage', 'ram', 'vcpu')
    def test_instance_cannot_be_created_if_tenant_service_project_link_quota_has_been_exceeded(self, quota):
        payload = self.get_valid_data()
        self.openstack_settings.scope.service_project_link.set_quota_limit(quota, 0)
        response = self.client.post(self.url, payload)
