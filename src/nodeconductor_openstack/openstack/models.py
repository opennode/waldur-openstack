...
class Tenant(structure_models.PrivateCloud):
    ...
    def _set_tenant_quotas(self, template):
        # TODO [TM:8/21/17] remove OpenStack dependency from Assembly
        ...
        for quota_name, component_type in packages_models.OpenStackPackage.get_quota_to_component_mapping().items():
            self.set_quota_limit(quota_name, components[component_type])

    def _set_tenant_extra_configuration(self, template):
        ...
        for component in template.components.all():
            self.extra_configuration[component.type] = component.amount
        self.save()

    def _set_template(self, template):
        self._set_tenant_quotas(template)
        self._set_tenant_extra_configuration(template)

    def set_package(self, template, service_settings=None):
        from nodeconductor_assembly_waldur.packages.models import OpenStackPackage

        self._set_template(template)
        if not service_settings:
            service_settings = self._create_service_settings()

        return OpenStackPackage.objects.create(
            tenant=self,
            template=template,
    ...
    def _create_service_settings(self):
        ...
        from nodeconductor_openstack.openstack_tenant.apps import OpenStackTenantConfig
        from nodeconductor_openstack.openstack_tenant import models as openstack_tenant_models
        service_settings_type = OpenStackTenantConfig.service_name
        ...
        openstack_tenant_models.OpenStackTenantServiceProjectLink.objects.create(
            service=service,
            project=self.service_project_link.project,
        )
