...
from .tasks import (PollRuntimeStateTask, PollDeletionTask, ForceDeleteDRBackupTask,
    ...
    def get_task_signature(cls, volume, serialized_volume, **kwargs):
        if volume.backend_id:
            return chain(
                ...
                PollDeletionTask().si(serialized_volume, 'is_volume_deleted'),
            )
        else:
    ...
    def get_task_signature(cls, snapshot, serialized_snapshot, **kwargs):
        if snapshot.backend_id:
            return chain(
                ...
                PollDeletionTask().si(serialized_snapshot, 'is_snapshot_deleted'),
            )
        else:
    ...
    def get_task_signature(cls, dr_backup, serialized_dr_backup, force=False, **kwargs):
        ...
        for volume_backup in dr_backup.volume_backups.all():
            ...
            deletion_tasks.append(chain(
                ...
                PollDeletionTask().si(serialized, 'is_volume_backup_deleted'),
    ...
    def get_task_signature(cls, instance, serialized_instance, force=False, **kwargs):
        if instance.backend_id:
            return chain(
                ...
                PollDeletionTask().si(serialized_instance, 'is_instance_deleted'),
            )
        else:
